...
*/
    ...
    tidied =  removeDiacritics(tidied);
    return tidied; 
}


function matches(results, inputString){
/* 
...
*/
    ...
    $.each(results, function(index, winery)){
        ...
        if (tidyName.indexOf(inputString) > -1 ){
            searchResults.push(winery);
        }
    }
    return searchResults;
}

var names = ["32 Winds", "Aliénor", "Clos Saint-Martin", "Gracia", "Château Milon", "F.Thienpont", "Domaine d’Ardhuy", "Vieux Château Certan", "Dr. Bürklin-Wolf"];
assert.deepEqual(matches(names, "Gracia"), ["Gracia"], "expected [\"Gracia\"]");
assert.deepEqual(matches(names, "gracia"), ["Gracia"], "expected [\"Gracia\"]");
